...
const EmployeeAnalytics = () => {
    ...
    // };
    const fetchTotalIncome = () => {
        const orderTimes = { breakfast: 0, lunch: 0, dinner: 0 };
        const today = new Date();
    
        const pad = num => num.toString().padStart(2, '0');
        const formatDateParts = (type) => {
        ...
        const isWithinTimeRange = (time, start, end) => time >= start && time <= end;
    
        const categorizeOrderTimes = (orders) => {
            orders.forEach(order => {
                const pickupTime = parseInt(order.pickup_time.slice(-4));
                //console.log("pickup time:", order.pickup_time);
    
                if (isWithinTimeRange(pickupTime, 500, 1159)) {
                    orderTimes.breakfast++;
                } else if (isWithinTimeRange(pickupTime, 1200, 1659)) {
                    orderTimes.lunch++;
                } else {
                    orderTimes.dinner++;
                }
        ...
        axios.get(`http://localhost:8000/order/byrestaurant/${restaurantID}`)
            .then(response => {
                ...
                const datePrefix = formatDateParts(selectedFilter);
                const filteredOrders = datePrefix
                    ? completedOrders.filter(order => order.pickup_time.startsWith(datePrefix))
                    : completedOrders;
                ...
                categorizeOrderTimes(filteredOrders);
            })
            .catch(error => {
                console.error("Error fetching orders:", error);
            });
            ...
            <div className="bg-gray-100 p-4 rounded-lg shadow-md flex flex-col flex items-center justify-center">
                <div className="flex items-center space-x-4">
                    ...
                    <button
                        ...
                        className="h-8 px-4 m-2 text-sm text-indigo-100 transition-colors duration-150 bg-green-500 hover:bg-green-600 rounded-lg focus:shadow-outline"
                    >
                        Apply Filter
                    </button>
                </div>
            ...
            <div className="min-w-screen flex flex-col min-h-screen flex items-center justify-center px-5 py-5">
                <div className="bg-slate-50 text-black-500 rounded shadow-xl py-5 px-5 ">
                    <div className="flex w-full">
                        <h3 className="text-xl font-bold leading-tight flex-1">TOTAL INCOME:</h3>
                    </div>
                    <div className="flex w-full">
                        <h3 className="text-2xl font-semibold leading-tight flex-1">${totalIncome.toFixed(2)}</h3>
                    </div>
            </div>
            <div className="p-10">
